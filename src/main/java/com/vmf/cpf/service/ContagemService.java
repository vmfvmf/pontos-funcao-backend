...
import com.vmf.cpf.mappers.ContagemMapper;
...
public class ContagemService extends AbstractService<ContagemDto, Contagem> {
	...
	public void prepareToSave(Contagem entity) {
		entity.getArquivosReferenciados().forEach(ContagemItemArquivoReferenciado::calcularPontosFuncao);
		entity.getTransacoes().forEach(ContagemItemTransacao::calcularPontosFuncao);
	...
	private Integer calcularTotalPontosFuncao(Contagem entity) {
		return entity.getArquivosReferenciados().stream()
				.reduce(0, (subTotal, arquivo) -> subTotal + arquivo.getPontosFuncao(),
				Integer::sum)
				+ entity.getTransacoes().stream()
				.reduce(0, (subTotal, transacao) -> subTotal + transacao.getPontosFuncao(), Integer::sum);
	}
